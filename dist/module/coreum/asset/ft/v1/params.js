...
export const Params = {
    encode(message, writer = new BinaryWriter()) {
        if (message.issueFee !== undefined) {
            Coin.encode(message.issueFee, writer.uint32(10).fork()).join();
        }
        if (message.tokenUpgradeDecisionTimeout !== undefined) {
            Timestamp.encode(toTimestamp(message.tokenUpgradeDecisionTimeout), writer.uint32(18).fork()).join();
        }
        if (message.tokenUpgradeGracePeriod !== undefined) {
            Duration.encode(message.tokenUpgradeGracePeriod, writer.uint32(26).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
