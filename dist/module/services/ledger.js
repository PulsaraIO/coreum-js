...
class Message {
    static new(props) {
        return String.raw `{"account_number":"${props.accountNumber}","chain_id":"coreum-mainnet-1","fee":{"amount":[{"amount":"5000","denom":"ucore"}],"gas":"200000"},"memo":"${props.memo || ""}","msgs":${JSON.stringify(props.msgs.map((m) => ({ type: m.typeUrl, value: m.value })))},"sequence":"${props.sequence}"}`;
