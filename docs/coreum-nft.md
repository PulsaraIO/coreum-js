...
```typescript
import { ClassFeature } from "tx-js";
```
...
```typescript
import { Client, NFT, ClassFeature, parseFloatToRoyaltyRate } from "tx-js";
...
```typescript
const balance = await client.queryClients?.nftbeta.balance("myclassid", "core1...");
