...
export const GenesisState = {
    encode(message, writer = new BinaryWriter()) {
        ...
        for (const v of message.classDefinitions) {
            ClassDefinition.encode(v, writer.uint32(18).fork()).join();
        }
        for (const v of message.frozenNfts) {
            FrozenNFT.encode(v, writer.uint32(26).fork()).join();
        }
        for (const v of message.whitelistedNftAccounts) {
            WhitelistedNFTAccounts.encode(v, writer.uint32(34).fork()).join();
        }
        for (const v of message.burntNfts) {
            BurntNFT.encode(v, writer.uint32(42).fork()).join();
        }
        for (const v of message.classWhitelistedAccounts) {
            ClassWhitelistedAccounts.encode(v, writer.uint32(50).fork()).join();
        }
        for (const v of message.classFrozenAccounts) {
            ClassFrozenAccounts.encode(v, writer.uint32(58).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
